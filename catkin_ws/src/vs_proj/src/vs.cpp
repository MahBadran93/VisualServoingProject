...
private:
  ...
  vpServo task;
  // Current and desired visual feature associated to the x coordinate of the
  // point
  vpFeaturePoint s_x, s_xd;
  vpFeatureDepth s_Z, s_Zd;

  vpCameraParameters cam;
  bool Stream_info_camera; // Is equal to one if we received the information
                           // about the camera
  double depth;
  double Z, Zd;
  double lambda;
...
void VS::poseCallback(const geometry_msgs::PoseStampedConstPtr &msg) {
  ...
  if (!Stream_info_camera) // We check if the streaming of images is started or
                           // not
  {
  ...
  try {
    ...
    // Update log(Z/Z*) feature. Since the depth Z change, we need to update the
    // intection matrix
